...
export function useCotizacionForm() {
    ...
    (async () => {
      try {
        ...
        if ((clientes.total ?? 0) === 0) {
          showToast("Primero debes registrar al menos un cliente antes de crear una cotización.", "warning");
          navigate(-1);
          return;
        }
        if ((items.total ?? 0) === 0) {
  ...
  useEffect(() => {
    if (!isEditing) return;
    const run = async () => { await loadCotizacion(); };
    run();
  ...
  const guardar = async () => {
    ...
    try {
      ...
      if (isEditing && cotizacionId) {
        ...
        navigate(`/cotizaciones/editar/${result.id}`);
      }
    } catch (error) {
      showToast(error.message, "error");
    } finally {
